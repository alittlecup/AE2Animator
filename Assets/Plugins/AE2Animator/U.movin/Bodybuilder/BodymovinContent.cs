...
namespace u.movin
{
    public struct BodymovinContent
    {
                    ...
                    for (int c = 0; c < n["assets"].Count; c++)
                    {
                        ...
                        if (a["id"] == layer.refId)
                        {
                            if (a["w"] != null)
                            {
                                ...
                                b.layers[j].imgPath = (""+a["p"]).Split('.')[0];
                            }
        ...
        public static BodymovinLayer ParseLayer(JSONNode d)
        {
            ...
            ParserLayerProperties(ref i, d);
            ParseShapes(ref i, d);
            ParseEffects(ref i, d);
            return i;
        }



        public static void ParserLayerProperties(ref BodymovinLayer b, JSONNode n)
        {
              b.nm = n["nm"];
            b.parent = n["parent"];
            b.ind = n["ind"];
        ...
        public static void ParseShapes(ref BodymovinLayer b, JSONNode n)
        {
            ...
            for (int i = 0; i < n["shapes"].Count; i++)
            {
                ...
                BodymovinShape s = new BodymovinShape {ty = d["ty"],ix=d["ix"]};
                ParseShapeItems(ref s, d);
                b.shapes[j] = s;
                j++;
            }
        }
        public static void ParseEffects(ref BodymovinLayer b, JSONNode n)
        {
            b.effectSets = new BodymovinEffectProperties[n["ef"].Count];
            int j = 0;
            for (int i = 0; i < n["ef"].Count; i++)
            {
                JSONNode d = n["ef"][i];
                BodymovinEffectProperties ef = new BodymovinEffectProperties() {ty = d["ty"]};
                if (ef.ty == 21)//填充
                {
                    ParseEffectItems(ref ef,d);
                }
                b.effectSets[j] = ef;
                j++;
            }
        }
        public static void ParseEffectItems(ref BodymovinEffectProperties b, JSONNode n)
        {
            b.ef = new BodymovinEffectProperties[n["ef"].Count];
            int j = 0;
            for (int i = 0; i < n["ef"].Count; i++)
            {
                JSONNode d = n["ef"][i];
                BodymovinEffectProperties ef = new BodymovinEffectProperties()
                {
                    ty = d["ty"],
                    nm = d["nm"],
                    mn = d["mn"],
                ...
                if (ef.nm =="颜色")//颜色
                {
                    var colorAnimated = d["v"]["a"] == 1;
                    if (colorAnimated)
                    {
                        b.fillColorSets = new BodymovinAnimatedProperties[d["v"]["k"].Count];
                        for (int k = 0; k < d["v"]["k"].Count; k++)
                        {
                            JSONNode keyFrame = d["v"]["k"][k];
                            b.fillColorSets[k] = new BodymovinAnimatedProperties()
                            {
                                t = keyFrame["t"],
                                s = new Vector3(keyFrame["s"][0], keyFrame["s"][1], keyFrame["s"][2])
                            };
                        }
                    }
                    else
                    {
                        b.fillColorSets = new[]
                        {
                            new BodymovinAnimatedProperties()
                            {
                                t = 0,
                                s = new Vector3(d["v"]["k"][0],d["v"]["k"][1],d["v"]["k"][2])
                            }
                        };
                    }
                }
                ...
                if (ef.nm == "不透明度")//颜色
                {
                    var opacityAnimated = d["v"]["a"] == 1;
                    if (opacityAnimated)
                    {
                        b.opacitySets = new BodymovinAnimatedProperties[d["v"]["k"].Count];
                        for (int k = 0; k < d["v"]["k"].Count; k++)
                        {
                            JSONNode keyFrame = d["v"]["k"][k];
                            ef.opacitySets[k] = new BodymovinAnimatedProperties()
                            {
                                t = keyFrame["t"],
                                sf = keyFrame["s"][0]
                            };
        ...
        private static void ParserEffectItemProperties(ref BodymovinAnimatedProperties[] sets,JSONNode d)
        {
            var opacityAnimated = d["v"]["a"] == 1;
            if (opacityAnimated)
            {
                sets = new BodymovinAnimatedProperties[d["v"]["k"].Count];
                for (int k = 0; k < d["v"]["k"].Count; k++)
                {
                    JSONNode keyFrame = d["v"]["k"][k];
                    sets[k] = new BodymovinAnimatedProperties()
                    {
                        t = keyFrame["t"],
                        s = new Vector3(keyFrame["s"][0], keyFrame["s"][1], keyFrame["s"][2])
                    };
        ...
        public static void ParseShapeItems(ref BodymovinShape b, JSONNode n)
        {
            int j = 0;
    ...
    public class BodymovinLayer
    {
        ...
        public BodymovinEffectProperties[] effectSets;
    ...
    public struct BodymovinEffectProperties
    {
        public int ty;
        public string nm;
        public string mn;
        public int ix;
        public BodymovinEffectProperties[] ef;
        public BodymovinAnimatedProperties[] fillColorSets;
        public BodymovinAnimatedProperties[] opacitySets;
    }
}