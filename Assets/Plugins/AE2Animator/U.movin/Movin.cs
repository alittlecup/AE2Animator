...
public class Movin
{
    ...
    public Movin(Transform parent, string path, int sort = 0, float scale = 1f, float strokeWidth = 0.5f, bool loop = false, float quality = 0.4f)
    {
    ...
    private void MovinInit(string path, int sort = 0, float scale = 1f, float strokeWidth = 0.5f, bool loop = true, float quality = 0.4f){
        ...
        for (int i = 0; i < content.layers.Length; i++) {
            MovinLayer layer = new MovinLayer(this.container.GetComponent<RectTransform>(), content.layers[i], "",content.layers.Length - i);
