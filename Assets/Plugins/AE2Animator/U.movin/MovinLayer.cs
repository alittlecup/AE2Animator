...
namespace u.movin
{
    public class MovinLayer
    {
        ...
        public MovinLayer(RectTransform rectTransform, BodymovinLayer layer, string imageFolder,int sort = 0)
        {
            this.root = rectTransform;
            this.content = layer;
            this.sort = sort;

            gameObject = GameObject.Instantiate(Resources.Load<GameObject>("Image"),root,false);
            gameObject.name = content.prefabName();
            ...
            if (layer.width != 0)
            {
                ...
                image.sprite = Resources.Load<Sprite>(imageFolder+"/"+layer.imgPath);
            }
        ...
        public void SetKeyframe(ref MotionProps prop, BodymovinAnimatedProperties[] set, int k = 0)
        {
            ...
            if (content.type == 2)
            {
                ...
                prop.endValue = v3 ? set[k].s : new Vector3(set[k].sf, 0, 0);
            }
            else
            {
                prop.startValue = v3 ? set[k].s : new Vector3(set[k].sf, 0, 0);
        ...
        public void UpdateProperty(float frame, ref MotionProps m, BodymovinAnimatedProperties[] set)
        {
            ...
            if (set == content.positionSets) {
                transform.anchoredPosition = Value3(m, ease) + positionOffset;
